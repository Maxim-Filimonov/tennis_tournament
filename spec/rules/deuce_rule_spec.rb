...
describe Tournament::Rules::DeuceRule do
  ...
  it 'does not apply when at least one player scored less than 3' do
    rule = described_class.new(instance_double('Scoreboard', scores: {'p1' => 2, 'p2' => 2}))

    rule.should_not be_apply
  end
  it 'applies when both players scored the same' do
    rule = described_class.new(instance_double('Scoreboard', scores: {'p1' => 4, 'p2' => 4}))

    rule.should be_apply
  end
  it 'applies when both players scored 3' do
